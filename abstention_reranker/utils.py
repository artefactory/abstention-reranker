...
def load_reranking_dataset(path):
    ...
    try:
        queries, positives, negatives = dataset['test']['query'], dataset['test']['positive'], dataset['test']['negative']
    except:
...
def process_dataset(queries, positives, negatives, num_docs_pr=10, max_num_pos_pr=5, random_seed=42):
    ...
    for query, positive, negative in zip(queries, positives, negatives):
        ...
        # sample docs
        if len(set(positive) & set(negative)) == 0: # check if there is an intersection between positive and negative docs
