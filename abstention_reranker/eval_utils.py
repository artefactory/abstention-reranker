...
def evaluate_strategies_on_benchmark(all_data, abstention_rates, dataset_names, model_names, metric_names, methods, random_seeds, test_size=0.2, alpha=0.1, quantile_bad=0.25, quantile_good=0.75, ref_subsample_size=None):
    strat_evals = {model_name: {dataset_name: {metric: {method: np.zeros((len(random_seeds), len(abstention_rates))) for method in ['oracle', 'random'] + methods} for metric in metric_names} for dataset_name in dataset_names} for model_name in model_names}
    ...
    for model_name in model_names:
        for dataset_name in dataset_names:
            ...
            for metric in metric_names:
                ...
                for i, seed in enumerate(random_seeds): # multiple runs (different seeds)
                    rel_scores_ref, rel_scores_test, metrics_ref, metrics_test = train_test_split(rel_scores, metrics, test_size=test_size, random_state=seed) # ref/test split 
                    
                    if ref_subsample_size is not None: # subsample ref set when relevant
                        np.random.seed(seed)
                        sampled_rows = np.random.choice(rel_scores_ref.shape[0], size=max(1, round(rel_scores_ref.shape[0] * ref_subsample_size)), replace=False)
                    ...
                    strat_evals[model_name][dataset_name][metric]['oracle'][i] = oracle_eval
                    strat_evals[model_name][dataset_name][metric]['random'][i] = oracle_eval[0]
                    
                    for method in methods: # evaluate methods    
                        conf_scorer = AbstentionReranker(method, metric, alpha, quantile_bad, quantile_good) # initialize confidence scorer
                        conf_scorer.get_scorer(rel_scores_ref, metrics_ref) # fit scorer (when relevant)
                        conf_scores_test = conf_scorer.compute_confidence_scores(rel_scores_test) # compute confidence scores
                        strat_eval = evaluate_strategy(conf_scores_test, metrics_test, abstention_rates) # evaluate strategy
                        strat_evals[model_name][dataset_name][metric][method][i] = strat_eval
...
def compute_naucs(strategy_evaluations, abstention_rates, dataset_names, model_names, metric_names, methods):
    naucs_df = pd.DataFrame(
        index=pd.MultiIndex.from_tuples([(d, m) for d in dataset_names for m in metric_names], names=['Dataset', 'Metric']),
        columns=pd.MultiIndex.from_tuples([(mo, me) for mo in model_names for me in methods], names=['Model', 'Method']))

    for model_name in model_names:
        for dataset_name in dataset_names:
            for metric in metric_names:
                auc_oracle = auc(abstention_rates, strategy_evaluations[model_name][dataset_name][metric]['oracle'].mean(axis=0))
                auc_random = auc(abstention_rates, strategy_evaluations[model_name][dataset_name][metric]['random'].mean(axis=0))
                
                for method in methods:
                    avg_perfs = strategy_evaluations[model_name][dataset_name][metric][method].mean(axis=0)
                    naucs_df.loc[(dataset_name, metric), (model_name, method)] = (auc(abstention_rates, avg_perfs) - auc_random) / (auc_oracle - auc_random)
    
    return naucs_df


def evaluate_raw_performance_on_benchmark(all_data, dataset_names, model_names, metric_names):
    raw_perf_df = pd.DataFrame(index=model_names, columns=pd.MultiIndex.from_tuples([(dataset_name, metric) for dataset_name in dataset_names for metric in metric_names], names=['Dataset', 'Metric']))
    ...
    for model_name in model_names:
        for dataset_name in dataset_names:
            ...
            for metric in metric_names:
                raw_perf_df.loc[model_name, (dataset_name, metric)] = evaluate_instances(rel_scores, targets, metric).mean() # evaluate instances
...
def compute_abstention_raw_performance_correlations(raw_perfs, naucs):
    ...
    for dname in dataset_names:
        corr_df.loc[dname, 'Correlation'] = pearsonr(raw_perfs[dname].values.flatten(), naucs.loc[dname].values.flatten())[0]
...
def make_calibration_study(all_data, abstention_rates, dataset_name, model_name, metric, methods, random_seeds, test_size=0.2, alpha=0.1, quantile_bad=0.25, quantile_good=0.75):
    ...
    for seed in random_seeds:
        ...
        for method in methods:
            conf_scorer = AbstentionReranker(method, metric, alpha, quantile_bad, quantile_good) # initialize confidence scorer
            conf_scorer.get_scorer(rel_scores_ref, metrics_ref) # fit scorer (when relevant)
            conf_scores_ref = conf_scorer.compute_confidence_scores(rel_scores_ref) # compute confidence scores for ref set
            conf_scores_test = conf_scorer.compute_confidence_scores(rel_scores_test) # compute confidence scores for test set
            ...
            for i, (rate, threshold) in enumerate(zip(abstention_rates, abstention_thresholds)):            
                thold_calibration_study[method][seed, i] = rate - len(conf_scores_test[conf_scores_test < threshold]) / len(conf_scores_test)
                perf_calibration_study[method][seed, i] = metrics_ref[conf_scores_ref >= threshold].mean() - metrics_test[conf_scores_test >= threshold].mean()
    
    # Summarize results in a summary data frame
    calib_df = pd.DataFrame(index=pd.MultiIndex.from_tuples([(study, method) for study in ['Abstention Rate', f'm{metric}'] for method in methods]), columns=abstention_rates)
...
def make_ref_size_study(all_data, abstention_rates, dataset_names, model_name, metric, methods, random_seeds, test_size=0.2, alpha=0.1, quantile_bad=0.25, quantile_good=0.75):
    ref_subsample_sizes = np.array([0.5**k for k in range(10)])
    ref_size_study = {dataset_name: {method: np.zeros((len(random_seeds), len(ref_subsample_sizes))) for method in methods} for dataset_name in dataset_names}
    ...
    for j, size in enumerate(ref_subsample_sizes):
        for i, seed in enumerate(random_seeds):
            strat_evals = evaluate_strategies_on_benchmark(all_data, abstention_rates, dataset_names, [model_name], [metric], methods, [seed], test_size, alpha, quantile_bad, quantile_good, size)
    ...
    for dataset_name in dataset_names:
        ref_sizes[dataset_name] = np.maximum(np.round(all_data[model_name][dataset_name]['scores'].shape[0] * ref_subsample_sizes * (1-test_size)), 1)
...
def get_minimum_reference_sizes(ref_size_study, ref_sizes, ref_free_method, ref_based_method):
    ...
    for dataset_name in dataset_names:
        min_ref_sizes.loc[dataset_name, 'Break-Even'] = int(ref_sizes[dataset_name][ref_size_study[dataset_name][ref_based_method].mean(axis=0) > ref_size_study[dataset_name][ref_free_method].mean(axis=0)][-1])
...
def make_runtime_study(model_path, dataset_path, conf_scorer, num_trials=100, seed=0):
    ...
    runtime_study = pd.DataFrame(0., index=range(num_trials), columns=['Relevance scores', 'Confidence scores'])
    np.random.seed(seed)
    ...
    runtime_study_summary['Extra Time'] = runtime_study_summary['Confidence scores'] / runtime_study_summary['Relevance scores']

    return runtime_study_summary


def make_domain_adaptation_study(all_data, abstention_rates, dataset_names, model_names, metric_names, methods, alpha=0.1, quantile_bad=0.25, quantile_good=0.75):
    dom_adap_study = []

    for dname_ref in dataset_names:
        dom_adap_study_dat = {model_name: {dname_test: {metric: {method: None for method in methods + ['oracle', 'random']} for metric in metric_names} for dname_test in dataset_names} for model_name in model_names}
        ...
        for model_name in model_names:
            ...
            for dname_test in dnames_test:
                ...
                for metric in metric_names:
                    ...
                    dom_adap_study_dat[model_name][dname_test][metric]['random'] = np.array([[oracle_eval[0]]*len(abstention_rates)])
        ...
        naucs_df.index = pd.MultiIndex.from_tuples([(dname_ref, dname_test, metric) for dname_test, metric in naucs_df.index], names=['Reference set', 'Test set', 'Metric'])
        dom_adap_study.append(naucs_df)

    return pd.concat(dom_adap_study)


def make_instance_qualification_study(all_data, abstention_rates, dataset_names, model_names, metric_names, methods, quantiles, random_seeds, test_size=0.2, alpha=0.1):
    inst_qual_study = {model_name: {dataset_name: {metric: {method: np.zeros((len(random_seeds), len(quantiles))) for method in methods} for metric in metric_names} for dataset_name in dataset_names} for model_name in model_names}

    for j, quantile in enumerate(quantiles):        
        quantile_bad, quantile_good = quantile, 1-quantile
        
        for i, seed in enumerate(random_seeds):
            strat_evals_quantile = evaluate_strategies_on_benchmark(all_data, abstention_rates, dataset_names, model_names, metric_names, methods, [seed], test_size, alpha, quantile_bad, quantile_good, None)                        
            naucs_quantile = compute_naucs(strat_evals_quantile, abstention_rates, dataset_names, model_names, metric_names, methods)
            
            for model_name in model_names:
                for dataset_name in dataset_names:
                    for metric in metric_names:
                        for method in methods:
                            inst_qual_study[model_name][dataset_name][metric][method][i,j] = naucs_quantile.loc[(dataset_name, metric), (model_name, method)]
    
    return inst_qual_study


def make_linreg_weights_analysis(all_data, dataset_names, model_names, metric_names, alpha=0.1):
    weights_analysis = {model_name: {dataset_name: {metric: None for metric in metric_names} for dataset_name in dataset_names} for model_name in model_names}
