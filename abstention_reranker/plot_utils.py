...
colors_dict = {'max': 'orange', 'std': 'red', '1-2': 'green', 'linreg': 'blue', 'logreg': 'purple', 'mahalanobis': 'black'}
markers_dict = {'max': 'o', 'std': 'o', '1-2': 'o', 'linreg': 'd', 'ridge': 'd', 'logreg': 'd', 'mahalanobis': 'd'}


def plot_performance_vs_abstention(strategy_evaluations, abstention_rates, dataset_name, model_name, metric_names, methods, savefig=False, path=None):
    ...
    for i, metric in enumerate(metric_names):
        plt.subplot(1, 3, i+1)
        plt.plot(abstention_rates, strat_eval_mod_dat[metric]['oracle'].mean(axis=0), linestyle='dashed', c='gray', label=r'$u^{*}$')
        plt.plot(abstention_rates, strat_eval_mod_dat[metric]['random'].mean(axis=0), linestyle='dotted', c='gray', label=r'$\tilde{u}$')
        
        for method in methods:
            plt.plot(abstention_rates, strat_eval_mod_dat[metric][method].mean(axis=0), c=colors_dict[method], marker=markers_dict[method], markersize=5, label='$u_{' + method[:3] + '}$')
...
def plot_naucs_vs_ref_size(ref_size_study, ref_sizes, dataset_names, methods, savefig=False, path=None):
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        for method in methods:
            ...
            plt.plot(ref_sizes[dataset_name], naucs_mean, color=colors_dict[method], marker=markers_dict[method], markersize=5, label='$u_{' + method[:3] + '}$')
            plt.fill_between(ref_sizes[dataset_name], naucs_mean - naucs_std, naucs_mean + naucs_std, color=colors_dict[method], alpha=0.3)
...
def plot_naucs_vs_qualification_threshold(inst_qual_study, quantiles, model_name, dataset_names, metric, methods, savefig=False, path=None):
    fontsize=16
    sns.set(style='white', palette='bright')
    plt.figure(figsize=(7,10))
    ...
    for i, dataset_name in enumerate(dataset_names):
        ...
        for method in methods:
            ...
            plt.plot(quantiles, naucs_mean, color=colors_dict[method], marker=markers_dict[method], markersize=5, label='$u_{' + method[:3] + '}$')
            plt.fill_between(quantiles, naucs_mean - naucs_std, naucs_mean + naucs_std, color=colors_dict[method], alpha=0.3)
